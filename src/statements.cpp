...
IfStmt::IfStmt(expr condition, std::shared_ptr<Stmt> then, std::shared_ptr<Stmt> elsestmt) : condition(condition), then(then), elsestmt(elsestmt) {}
...
ClassDecl::ClassDecl(Token name, std::vector<FunDecl> methods, expr base) : name(name), methods(methods), base(base) {}
Label::Label(Token name) : name(name) {}
Goto::Goto(Token name) : name(name) {}
Include::Include(Token tok, std::string path) : tok(tok), path(path) {}
